...
> **Nota de contexto:** este documento é a referência de design usada como base para os tokens de cor/tipografia do Piano Urbano (ver `app/globals.css`). O nome "Starbucks" é só a origem do vocabulário visual (verdes, dourado, tipografia arredondada) — o produto final não tem qualquer relação com a marca Starbucks. Ao pedir novos componentes de UI para este projeto, use os nomes de tokens já mapeados em `app/globals.css` (`--brand`, `--house`, `--gold`, etc.), não os nomes originais deste documento.
